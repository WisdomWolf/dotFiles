function create-db-user
    ...
    if set -q _flag_help; or test (count $argv) -lt 1
        echo "Usage: create-db-user <username> [-d database] [-p password] [-u uri] [-n op-item-name]"
        ...
        echo "  -n, --op-item-name  1Password item name (defaults to <username>-db)"
        return 0
    end
    ...
    if string match -q 'postgresql://*' $_flag_uri; or string match -q 'postgres://*' $_flag_uri
        ...
        if psql $_flag_uri -tAc "SELECT 1 FROM pg_roles WHERE rolname='$username';" | grep -q 1
    ...
    if string match -qi 'y' $confirm
        if op item get $op_item_name --vault Homelab 2>/dev/null
            ...
            or echo "Failed to save to 1Password" >&2
        end
    end
end
